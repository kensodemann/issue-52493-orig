...
describe('SessionVaultService', () => {
  ...
  beforeEach(() => {
    ...
    (mockVault.onLock as jasmine.Spy).and.callFake(
      (callback: () => void) => (onLockCallback = callback),
    );
    (mockVault.onPasscodeRequested as jasmine.Spy).and.callFake(
      (callback: (flag: boolean) => Promise<void>) =>
        (onPassocodeRequestedCallback = callback),
    );
  ...
  describe('initialize', () => {
    it('initializes the vault', async () => {
      ...
      expect(mockVault.initialize).toHaveBeenCalledTimes(1);
    });
  });
  ...
  describe('after initialization', () => {
    ...
    describe('can unlock', () => {
      [
        ...
        it(`is ${!empty && locked} for ${empty} ${locked}`, async () => {
          (mockVault.isEmpty as jasmine.Spy).and.returnValue(
            Promise.resolve(empty),
          );
          (mockVault.isLocked as jasmine.Spy).and.returnValue(
    ...
    describe('onPasscodeRequested', () => {
      beforeEach(() => {
        (modal.onDidDismiss as jasmine.Spy).and.returnValue(
