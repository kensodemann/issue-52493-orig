...
describe('AuthenticationService', () => {
  ...
  beforeEach(() => {
    ...
    spyOn(AuthConnect, 'login').and.returnValue(
    ...
    spyOn(AuthConnect, 'buildAuthResult').and.callFake(() =>
      Promise.resolve(builtAuthResult),
    );
    ...
    TestBed.configureTestingModule({
      providers: [
        ...
        },
        {
          ...
          useValue: createSessionVaultServiceMock(),
        },
      ],
    });
  ...
  describe('is authenticated', () => {
    describe('if there is no auth result', () => {
      beforeEach(() => {
        ...
        spyOn(AuthConnect, 'isAccessTokenExpired').and.returnValue(
    ...
    describe('if there is an auth result', () => {
      beforeEach(() => {
        (sessionVault.getSession as jasmine.Spy).and.returnValue(
          testAuthResult,
        );
      });
      ...
      describe('if the access token is expired', () => {
        ...
        describe('if a refresh token exists', () => {
          beforeEach(() => {
            spyOn(AuthConnect, 'isRefreshTokenAvailable').and.returnValue(
              Promise.resolve(true),
            );
          });

          it('attempts a refresh', async () => {
            spyOn(AuthConnect, 'refreshSession').and.returnValue(
              Promise.resolve(refreshedAuthResult),
            );
            await authService.isAuthenticated();
          ...
          describe('when the refresh is successful', () => {
            ...
            it('saves the new auth result', async () => {
              ...
              expect(sessionVault.setSession).toHaveBeenCalledWith(
                refreshedAuthResult,
              );
            });
          ...
          describe('when the refresh fails', () => {
            beforeEach(() => {
              spyOn(AuthConnect, 'refreshSession').and.rejectWith(
    ...
    describe('if there is an auth result', () => {
      ...
      describe('if the access token is not expired', () => {
        ...
        it('resolves the access token', async () => {
          expect(await authService.getAccessToken()).toBe(
        ...
        describe('if a refresh token exists', () => {
          ...
          describe('when the refresh is successful', () => {
            ...
            it('resolves the new token', async () => {
              expect(await authService.getAccessToken()).toEqual(
                refreshedAuthResult.accessToken,
              );
            });
          });
  ...
  describe('logout', () => {
    ...
    describe('if there is no auth result', () => {
      ...
      it('builds an auth result', async () => {
        ...
        expect(AuthConnect.buildAuthResult).toHaveBeenCalledWith(
          jasmine.any(Auth0Provider),
          jasmine.any(Object),
          {},
        );
